...
const (
	protocolAckByteSize    = 1
	protocolAckSuccessCode = 1
	protocolAckIndex       = 0

	protocolMessageTypeBatch        uint8 = 1
	protocolMessageTypeFinish       uint8 = 2
	protocolMessageTypeWinnersQuery uint8 = 3

	protocolWinnersCountByteSize = 2
	protocolDNIByteSize          = 4
...
func (b *ProtocolBuilder) BuildFinishMessage(agencyID uint8) []byte {
	return []byte{protocolMessageTypeFinish, agencyID}
}

func (b *ProtocolBuilder) BuildWinnersQueryMessage(agencyID uint8) []byte {
	return []byte{protocolMessageTypeWinnersQuery, agencyID}
}
